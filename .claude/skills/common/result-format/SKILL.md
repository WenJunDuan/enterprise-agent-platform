...
## 结论措辞规范（`explanation` / `reasons` / `evidence_chain` 通用）

写得像财务 / 审计人员的正式审核意见：平实、专业、克制。

- **禁用夸张或口语化词**：硬伤、铁证、实锤、妥妥、铁定、坐实、必然是 等。改用中性表述：存在、疑似、不满足、缺少、与……不一致。
- **不要堆砌技术黑话**：如「Unix 纪元」「系统默认值占位符」这类实现细节。字段异常直接描述现象即可，例如「日期为 1970-01-01，疑似默认值或占位值」，不必解释其技术来源。
- **不要加自我点评式括注**：如「（典型占位数据）」「（非真实人员）」。事实写进 `finding`，定性交给 `verdict` 与 `reasons`，不在描述里反复贴标签。
- **同一问题只说一次**：不要给一个现象连套多个标签（占位 + 默认值 + 硬伤）。
- **定性留有余地**：除非证据确凿且规则支持，否则用「疑似 / 需人工核实」，不要替调查人员下「造假 / 伪造」的终局结论。
...
- 不要输出英文版审核意见
- 不要使用夸张或口语化措辞（硬伤 / 铁证 / 实锤 / 妥妥 等）
- 不要堆砌技术实现细节（如「Unix 纪元」）或自我点评式括注（如「（典型占位数据）」）

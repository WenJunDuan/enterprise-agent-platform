...
**取材纪律（硬性）**：服务端注入的底稿**就是你能取材的全部内容**，**禁止 `Glob` / `Read` 任何文件**。两种形态，都带 `### 文件:` 头与 `【第N页】` 页锚：①**整份底稿**（OCR/直读，已按评标目的重点还原评分/扣分表格）；②**按评分项检出的证据片段**（头部写明「按评分项检索到的证据片段」）——此形态下未注入的材料**不等于投标人未提供**，该项按**证据缺失**处理，不得据此判 0。例外只有两个：①某页读不清时**先带图问** `vision-page`（`uv run python .claude/skills/vision-page/vision.py <绝对路径> --page N --question "一个判定点"`），答不出或端点不可用再用 `ocr-page` 转写（`uv run python .claude/skills/ocr-page/ocr.py <绝对路径> [--pages N 或 N-M] [--seal]`）；两者都须为本目录内真实文件，不拼 shell 运算符；②**服务端在注入末尾开放补证工具时以那段为准**（未开放即本条禁令全效）。底稿被截断（`...[已省略…]...`）或材料未注入 → 相关项按 A3 **证据缺失**处理。

### 出处页号书写规则（硬性，页锚溯源）

底稿里页锚有两种坐标系，**出处必须照抄底稿里实际出现的那一种，不得互换、不得臆造**（证据片段形态：照抄**该片段自己的** `### 文件:` 与页锚，不跨片段拼）：
...
**默认不 spawn 子 agent**，本会话内连续完成 S0–S4；除上述例外不应有工具往返。

### S0 立案清点
- **不要 `Glob`**：文件清单直接从底稿的 `### 文件: <文件名>` 行读取（证据片段形态即各片段头，以在场片段头为准，不推断缺席文件），按文件名/内容分类：招标文件、投标文件各章节、投标人标识，形成清单（章节号 → 文件）。不要逐字深读，先建索引。
...
- 本项目的资格审查规则和评分标准**就在已注入的招标文件底稿里**。标题与章节因标书而异（正文某章 / 评标须知 / 前附表 / 附录皆有可能），**以本招标文件实际结构为准，不要预设"第三章"或某个固定标题**。把定位到的项目规则**直读解析**为本项目 `criteria`，对齐 `.claude/contracts/tender/criteria.schema.json`：
  ...
  - `items[]`：每项除 `{item, max, scoring_rule 原文, source_ref, tag, category}` 外，**必须判定该项评分方式 `score_mode` 并按方式提取结构化细则**（对齐 criteria.schema）：
    ...
    - **复合评分行 → 拆成多条 items**：招标文件一个评分行含多个**独立子规则**（如「基础响应分 + ▲加分」「驻场人员计分 + 资格证书计分」「质量体系 + 服务承诺 + 培训」）时，**拆成多条 criteria items**、各自取最贴切的 `score_mode`，各 item `max` 之和 = 原行满分（契约一项只允许一种 `score_mode`，**不要把扣减与加分混进同一项**）。例：「运营平台 24 分 = 功能响应 18 分（deduction）+ ▲检测报告加 6 分（additive）」→ 两条 items。
...
- （可选 · G2）读取/评分计划可写成结构化节点进 `extracted_data.plan`，对齐 `.claude/contracts/common/plan.schema.json`（`{step, intent, reads, tools, produces, tag}`）；平台只校验其形，形不符即丢弃该字段。
...
- 按 `extracted_data.criteria` 的 `eligibility_rules[]` 与各评分项所需证据，从底稿逐项定位相关投标章节，抽取评标所需事实，对齐 `.claude/contracts/tender/extract-result.schema.json`。**顺序：先资格审查证明（主体资格 / 资质证书 / 项目负责人资格 / 信用承诺 / 主体库材料），再取评分项所需的业绩、技术、报价等材料。**
  - **基本信息块（必产，抓自文件内容，不照抄上传时人填的字段）→ `extracted_data.basic_info`**：招标侧 `project_name`/`tender_no`/`tenderee`（抽自招标文件）、投标侧 `bidder_name`/`credit_code`/`legal_person`（抽自投标函、营业执照、盖章页原文），每项写 `{value, source:"文件名 第N页"}`；抽不到写 `value:null` + 已查页范围，不编造。同批数据钉入 `extracted_data.bidder_info`（对齐 `.claude/contracts/tender/bidder-info.schema.json`）。
  - **两条一致性判定必出**（缺即未完成）→ `basic_info.consistency:[{check, status:match/mismatch/manual, basis 含两侧页码}]`：①投标文件正文的投标人名 ↔ 上传申报名，不符要点名；②**应标一致性**：投标文件声称响应的项目名称/编号 ↔ 本次招标的项目名称/编号，**不一致即重大发现（投错标/套卷）**，给两侧页码，确认不符按 A5 / 废标 gate，读不清按 A3。
  - 拟派项目负责人：姓名 / 注册证号 / 出处（文件+页）
  - 业绩：每条 `项目名称 / 项目经理 / 出处（文件+页）`
  - 投标报价（**钉入 `extracted_data.bid_price` = `{amount: 数值, currency: "CNY"}`**，供后续多家价格横比统一收集）、章节-页码索引。**小写与大写金额两处原文各带页码必抽**（开标一览表 / 投标函）；抽不到要写查过哪些文件页，不得略过报价
  - 资格审查证据：按 `eligibility_rules[]` 逐条抽取对应材料出处，如营业执照、资质证书、安全生产许可证、项目负责人证书、劳动合同/社保证明、信用承诺/信用查询截图、主体库/动态监管要求等；依赖外部系统而投标文件未提供结果的记“需外部核验”。
...
- 其余一致性线索同样写进 `ambiguities`（如姓名写法不一致、业绩项目经理与拟派负责人疑似不同人）。
- 只抽事实，不在本步给分。
...
| A2 | 判罚·扣分·资格相关页底稿读不清（扫描 / 印章 / 截图） | 先用 `vision-page` **带图问**该页（一问一个判定点）；答不出或端点不可用再用 `ocr-page` **重识别**（印章页加 `--seal`）；仍不可读 → 落 A3 |
...
| A7 | 同上但仅**写法存疑同一人**（简繁 / 形近 / OCR 易混，如 王建国/玉建国） | `score:null` + `manual_review`(`data_conflict`) |
...
- **每项必须给结论，禁裸 manual_review**：每个资格项与评分项**三选一**——给分 / 有据判 0 / 待人工。判定标准仍**只看上表决策表**，本条只加输出义务：待人工的 `basis` 必须写全三段——**已找到什么**（文件+第N页）/ **缺什么** / **查过哪里**（已查的文件与页范围）。只写「需人工复核」而无这三段 = 未完成。
- **清单先行、逐条销项**：S1 已把 `eligibility_rules[]` + 评分 `items[]`（含各项分值）**全量罗列成清单**，S3 对着它**逐条销项**，每条落「要求→证据→页码→判定」四段。清单里有而 `eligibility_checks` / `scoring` 里没有 = **漏项**，本次评标未完成。
...
- 对照 S1 的 `criteria` 每一项，结合事实底稿（必要时按页锚点 `【第N页】` 回读原文）判定，写入 `extracted_data.scoring`，每项 `{item, max, score, status, score_mode, basis, …按 mode 的明细}`（`item`/`max`/`score_mode` 与 criteria 对应项一致）。**先过决策表定判 0 / manual，命中 A9 再按该项 `score_mode` 出分**：
  ...
  - `formula`（公式分）→ **以 `formula_spec` 为准、按变量闭合性判**（限价类本可单家算，别全丢人工）：
    ...
    - 价格分项 `basis` **必须先写报价结论再写待横比**（缺任一条即未完成）：①「本家报价小写 X 元、大写同额，**勾稽一致**，低于控制价，为有效报价」——大小写不符 / 缺一处 / 分项合价对不上总价，照实写「勾稽不一致：…」并按决策表处置；②「价格分**待横比合成**（评标基准价须全部投标报价汇总后统一计算）」。待横比 ≠ 什么都没判。
  - `pass_fail` → 满足得 `max`，否则按决策表（A8 判 0 / A3–A5 走 `manual_review`）。
- **证据效力核查（客观分通用，给分前过一遍）**：资质/人员证书有效期须覆盖投标截止日；业绩合同签订日期须在招标要求窗口内；招标要求盖章的截图/彩页核对**章的归属**（要求原厂公章的，投标人公章不算数，二者可能同时要求）；同一评分点的多本证书须归属同一人。效力**确认不成立**（如证书确认过期）按 A8/A9 计分；效力**存疑**（章模糊、关键字样读不清）走 A2 → 仍不可读落 A3。**证书有效期 / 公章归属 / 大写金额 / 检测报告标识属像素判定点：优先 `vision-page` 带图问，不要只采信 OCR 文字**（盖章扫描页常被转写成空或错字）。
- **废标/资格独立 gate**：资格审查已按最高优先级输出 `eligibility_checks`；再对照 S1 的 `rejection_rules` 逐条核查投标文件，命中写 `extracted_data.disqualification_hits:[{rule_id 回链, finding, confirmed, evidence:{source, quote}}]`。把"项目名不符"等记入 `disqualification_hits` + 相关项 `basis`，而非抹掉逐项评分。
  - **`confirmed` 是废标决断的闸**：仅当废标事实**已确认**（底稿可读、逐字可核、语义明确，如确认逾期/确认投错项目/确认资质缺失）才写 `confirmed:true` → 触发 `rejected`。**疑似 / 读不清 / 扫描截图未还原 / 自相矛盾 / 须人工登官网核验**的信号一律 `confirmed:false`——它只进 `risk_score` + `eligibility_checks.status:manual` 提示人工，**绝不触发 rejected**。典型反例：信用中国查询截图只读到页面标题（"…失信…名单"）读不全结果、且放在"未被列入"自证章节 → 常规理解是**自证清白**，`confirmed:false`，不得据此废标。另一类反例：**电子标项目**中投标函、开标一览表等文件可能由交易平台在线生成、不随导出文件袋——判「必交文件缺失」触发废标前，先看招标文件是否载明平台在线生成/系统填报机制；属此类的 `confirmed:false`、相关资格/符合项标 `manual`（写明「待核验平台生成机制」），但其承载的关键承诺（如投标有效期）须能在袋内其他材料找到落点，找不到仍记缺口。（读不清的判罚页先走 A2 重识别；重识别后仍不可读 → `confirmed:false`。）
- 一致性核验：业绩项目经理 vs 拟派项目负责人，按决策表 A6 / A7 二分决断，不给"或"。
- **证据定位准确性（硬要求，定位项必须 = 实际找到的）**：每条 `basis` / `evidence_chain` 的出处**只能引底稿里真实存在的页锚点 `【第N页】`**，且所引页**确实包含**你描述的内容——**严禁凭印象/猜测写页码**。⚠ **页码 N 必须取底稿中该原文正上方最近的 `【第N页】` 锚点数字（OCR 顺序页），不是投标文件正文里印刷的页码**——两者常因封面/目录/分册偏移而差几页，照搬印刷页号会被回查闸判 `page_mismatch`。写每条证据前自检一遍：「该原文/字段是否就在我所引的 `【第N页】`？」对不上就改到正确页或降为"未在底稿定位到"。出处统一写**「文件名 + 第N页 + 所在章节/标题」**（如「投标文件第6页《应答函》」；**带文件名，便于跨多文件归属**），`finding`（及 `deduction_hits/award_hits/disqualification_hits.evidence.quote`）摘所引页的**逐字原文片段**（**照抄底稿原文、勿转述/勿改写/勿缩写**），使定位可核验、带上下文。
  - ⚠ **服务端有确定性回查闸**：会把你引的每条 `quote` 拿去本案底稿逐字核对——引的是**底稿里真实存在的逐字原文**才算核实；编造/严重转述会被标 `evidence_unresolved` 并把该评分项降为人工复核。
...
- **综合意见口径**：`explanation` 可按「资格审查 / 价格分 / 商务客观分 / 技术主观分」四类分述：资格审查先说明是否通过及废标主因；价格分照 S3 的报价 `basis` 口径复述（有效性 + 大小写勾稽 + 待横比合成）；商务客观分说明可量化项的得分、扣分与依据；技术主观分**同样直接报分数和归档依据**（页数/覆盖度/要点命中），不逐项加免责语；**整单说明末尾统一一句**「主观评分项为按评标办法档次标准的评定，评标委员会可复核调整」即可。资格审查确认不通过时 `explanation` 开头直接写「资格审查不通过，按废标处理」，并说明评分明细仍逐项列示、但不参与有效投标排序；通过则写已有分数项合计与待补充确认的项。**不要要求或输出 `review_dimension` 字段**，展示维度由前端按 `criteria.items[]` 既有结构化字段派生。
...
- **口头总分必须 = 结构化 `scoring[]` 非 null `score` 之和**：`explanation` 里若写汇总分，只能加 `status:scored` 的项；`score:null`（manual）项**不计入**口头总分，单独表述「该项已估算 X 分，待人工/横比确认」。逐项分数以 `extracted_data.scoring` 为准，小结不再列手算式（避免与结构化分数不一致）。
...
5. `extracted_data.eligibility_checks` 为最高优先级资格审查结果，必须先于 `scoring` 产出；`extracted_data.scoring` 为逐评分项 `{item, max, score, status, score_mode, basis, pending_reason（仅 score=null 时必填）, …按 mode 的 deduction_hits/selected_band/award_hits}`。资格审查不计入合计，未判定评分项 `score:null` 不计入合计。废标/资格走 `extracted_data.disqualification_hits` / `eligibility_checks`（独立 gate，**不混入 scoring**）。
...
8. 结论须钉入 S2 基本信息块及其派生的 `extracted_data.bidder_info` / `tender_info`（分别对齐 `.claude/contracts/tender/bidder-info.schema.json` 与 `tender-info.schema.json`，后者取 `project_name`/`tender_no`/`tenderee` 子集即可，均附页锚 `source_refs`）。

## 单投标人边界与多投标人追加

- **本命令一次只评目录里的这一家投标人**（招标文件 + 该投标人投标文件）。**不要**尝试读取、比较其他投标人或既往评标结果——多投标人的横向比较、汇总、增量追加由上层前端 / server 负责。
- 因此 `requires_cross_bid_comparison` 项（价格分、有效投标数等）按决策表 A4 处理，本家数据照 S3 口径钉入 `extracted_data.bid_price`。

...
**取材纪律（硬性）**：服务端注入的底稿**就是你能取材的全部内容**，**禁止 `Glob` / `Read` 任何文件**。两种形态，都带 `### 文件:` 头与 `【第N页】` 页锚：①**整份底稿**（OCR/直读，已按评标目的重点还原评分/扣分表格）；②**按评分项检出的证据片段**（头部写明「按评分项检索到的证据片段」）——此形态下未注入的材料**不等于投标人未提供**，该项按**证据缺失**处理，不得据此判 0。唯一例外：某页读不清时用 `ocr-page` 重识别（`uv run python .claude/skills/ocr-page/ocr.py <绝对路径> [--pages N 或 N-M] [--seal]`，须为本目录内真实文件，不拼 shell 运算符）。底稿被截断（`...[已省略…]...`）或材料未注入 → 相关项按**证据缺失**处理（A3），不要补读。
...
### S0 立案清点
- **不要 `Glob`**：文件清单直接从底稿的 `### 文件: <文件名>` 行读取（证据片段形态即各片段头，清单以在场片段头为准，不推断缺席文件）。按文件名/内容分类：招标文件、投标文件各章节、投标人标识。
...
- 其余一致性线索同样写进 `ambiguities`，例如"拟派负责人姓名在不同文件写法不一致（王建国/玉建国）""所报业绩项目经理与拟派负责人疑似不一致"。
- 只抽事实，不在本步给分。

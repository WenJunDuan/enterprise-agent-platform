...
**取材纪律（硬性）**：服务端注入的底稿**就是你能取材的全部内容**，**禁止 `Glob` / `Read` 任何文件**。两种形态，都带 `### 文件:` 头与 `【第N页】` 页锚：①**整份底稿**（OCR/直读，已按评标目的重点还原评分/扣分表格）；②**按评分项检出的证据片段**（头部写明「按评分项检索到的证据片段」）——此形态下未注入的材料**不等于投标人未提供**，该项按**证据缺失**处理，不得据此判 0。例外只有两个：①某页读不清时用 `ocr-page` 重识别（`uv run python .claude/skills/ocr-page/ocr.py <绝对路径> [--pages N 或 N-M] [--seal]`，须为本目录内真实文件，不拼 shell 运算符）；②**服务端在注入末尾开放补证工具时以那段为准**（未开放即本条禁令全效）。底稿被截断（`...[已省略…]...`）或材料未注入 → 相关项按 A3 **证据缺失**处理。
...
**默认不 spawn 子 agent**，由你在本会话内连续完成 S0–S4；材料已全部注入，除上述两个例外外不应有工具往返。

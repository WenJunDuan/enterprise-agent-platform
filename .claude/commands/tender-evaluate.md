...
- **定位线索与排除（这步错则全错）**：先找最高优先级的资格/初步审查规则，再找评分表。资格规则常见标题包括 **资格审查 / 资格性审查 / 资格评审 / 初步评审 / 符合性审查 / 响应性评审 / 资格审查证明文件**；评分标准通常用**评分表**承载，列含「序号 / 评分点名称 / 评审标准 / 最高分 / 最低分 / 主客观分」或「评分项 / 分值 / 权重」。章节标题含 **开标 / 评标 / 评审 / 商务技术标 / 报价标** 也可能是评分标准来源，**不限于《评标办法》字样**（本类标书常把资格评审和评分表放在「第 X 部分 开标和评标」或第三章前附表里）。**关键排除**：详细评分标准是评标委员会对**投标文件**打分用的（商务技术 + 报价等评分项，合计 = 满分）；资格审查是并列的前置招标项，**不计入满分合计**。必须排除中标后的「考核方案 / 考核指标 / 季度考核 / 绩效考核 / 履约考核 / KPI」表——它们多挂在「附件·考核方案」、同样列分值，但属中标**后**履约阶段，**误取会让整套 criteria 取错**。定位后自检：评分 `items[].max` 之和是否 = 总分（如 70 + 30 = 100）；资格审查不得参与该合计。对不上 = 漏取或取错表，回去重定位。

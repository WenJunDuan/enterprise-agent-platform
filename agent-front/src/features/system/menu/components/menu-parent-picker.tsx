...
function filterExcludedNodes(
  ...
  return nodes.flatMap((node) => {
    if (excludedIds.has(node.menuId)) {
      return []
    }
...
export function MenuParentPicker({
  value,
  onChange,
  nodes,
  excludedIds,
  disabled,
}: MenuParentPickerProps) {
  const [open, setOpen] = useState(false)
  const [keyword, setKeyword] = useState('')
  const [expandedIds, setExpandedIds] = useState<Set<string>>(new Set())

  const availableNodes = filterExcludedNodes(nodes, excludedIds)
  const hasRealRootNode = findMenuById(availableNodes, '0') !== undefined
  const filteredNodes = filterMenuTree(availableNodes, keyword, 'all')
  const treeViewItems = buildMenuTreeViewItems(filteredNodes)
  const defaultExpandedIds = keyword.trim()
    ? collectExpandableIds(filteredNodes)
    : createDefaultExpandedMenuIds(availableNodes)
  const effectiveExpandedIds = ensureMenuPathExpanded(
  ...
  const selectedLabel =
    value === '0'
      ? hasRealRootNode
        ? getMenuRootLabel(availableNodes)
        : '顶级菜单'
      : (findMenuById(availableNodes, value)?.menuName ?? '请选择上级菜单')

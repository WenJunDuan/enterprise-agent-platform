...
# 片段形态（每块 ``### 文件:`` + 页锚 + 章节）也在这里交代一次：命令里的取材/出处纪律按
# "整份底稿"写，两处口径必须同源，否则模型按"没检出=没提供"判 0（review pass3 F3）。
_EVIDENCE_HEADER = (
    "=== 按评分项检索到的证据片段（服务端确定性检索，含出处与页锚）===\n"
    "说明：以下片段由服务端按 criteria 逐项从招标/投标底稿索引中检出，已跨项去重。"
    "每个片段自带 `### 文件: <文件名>` 头、`【第N页】` 页锚与 `【章节: …】`——出处**照抄该片段的**"
    "文件名与页号，不要跨片段拼。本次材料清单以这些片段头为准。"

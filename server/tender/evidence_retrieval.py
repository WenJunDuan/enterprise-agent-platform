"""证据层的按项检索与组装：查询构造 → 命中 → 沿文档顺序续接 → 受闭式账目约束地装配（KD1/KD2）。

与 ``evidence_index`` 分家的理由是变更理由不同：那里回答"**两层底稿怎么进索引**"（写入、
幂等、两层 file 标识），这里回答"**给定 criteria 怎么取出该注入哪些段落**"（查询串、每项
额度、跨项去重）。续接**走到哪停**又是独立一轴（S5 起已改写两次），故再拆去
``evidence_continuation``——三次都是同一条硬线逼出来的：合在一处越 300 行。
...
class EvidenceResult:
    ...
    # 逐项实际注入了多少 token，``(项名, token)`` 按检索顺序、**每项一行含零值**（S7/AC15）。
    # "证据变薄"没有专属故障码：它既不是 unresolved（真没有）也不是 truncated（有但没装下），
    # 而续接边界质量依赖邻项命中的疏密（邻项零命中→边界后移 / 噪音命中→伪边界提前截断 /
    # 末项之后无停止点）。这本逐项账是让上述三种变薄被看见的**唯一**量纲。
    item_tokens: list[tuple[str, int]] = field(default_factory=list)
...
def _item_evidence(
    hits: list[dict[str, Any]], *, conn: sqlite3.Connection, budget: int, stop_ids: set[str]
) -> Iterator[dict[str, Any]]:
    """Yield one item's evidence in document order: each hit, then the text up to the next hit."""
    for hit in hits:
        yield hit
        yield from continuation(hit, conn=conn, budget=budget, stop_ids=stop_ids)
...
def _assemble_item(
    ...
    *,
    ...
    item_tokens = 0
    has_evidence = False
    for hit in _item_evidence(hits, conn=conn, budget=plan.per_item_tokens, stop_ids=stop_ids):
...
def retrieve_evidence(
    ...
    *,
    ...
    Returns:
        ...
        ``evidence_unresolved`` 并显示实际用过的查询串（AC2/AC7）；``item_tokens``
        逐项留痕实际注入量，让"证据变薄"这类无故障码的降级同样可见（AC15）。
    """
    ...
    # 两遍式（S7）：先全项检索，再逐项组装。续接的停止点是**全项命中块的并集**，边检边组装
    # 的单遍循环拿不到这个全局视图。每项取几个 chunk 只在 InjectionPlan 里推导一次（P2/DRY）。
    searched = [
        (name, *_search_item(queries, conn=conn, limit=plan.chunks_per_item))
        for name, queries in items
    ]
    stop_ids = {hit["chunk_id"] for _, hits, _ in searched for hit in hits}

    result = EvidenceResult(plan=plan)
    seen: set[str] = set()
    for name, hits, tried in searched:
        if not hits:
            result.unresolved.append(UnresolvedItem(item=name, queries=tried))
            result.item_tokens.append((name, 0))
            continue
        ...
        result.item_tokens.append((name, item_tokens))
        if not has_evidence:

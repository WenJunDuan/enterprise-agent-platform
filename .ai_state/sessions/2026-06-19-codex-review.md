...
| Medium | `common/contract.py` 仍含 audit 专属逻辑（enrich/derivation/validate），未达 design "common 域中立、audit 自注册 processor" | ⏸️ **暂留**（已知取舍）。registry 已让其可行，但 init-rules(system) 无 server 侧模块承载注册，注册归属未定；契约一致性归 common 可接受。待后续 |
...
| Low | `deploy/prod/litellm_config.yaml` 硬编码 key | 用户已决定原样追踪；建议轮换（key 已在 git 历史） |

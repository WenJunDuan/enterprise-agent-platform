...
> 回填 2026-08-19：case-zj-live · mode=single · n=3（镜像 0818b3 + deepseek，
> project tp-7067a4ce23ca4db0，run 明细在 `/tmp/step2-baseline.log` 与任务表）。
> **三次跑命中/漏检逐条一致**——漏检是结构性的（D1/D2 病灶），非随机波动。

| 指标 | single 路径（n=3 中位｜极差） | itemized（各 Phase 后追记） |
|---|---|---|
| 墙钟 | **299s**（271–338s）——贴着 ≤5min 达标线 | |
| manual_review 数（计劣化） | **6**（全为 evidence_unresolved；正确待人工另 1 项单列） | |
| 跨文件缺陷召回 | **2/7 = 29%**（D1、D4 三跑全中；D2 仅弱键；D3/D5/D6/D7 全漏） | |
| 客观分准确率 | **0/3**（企业实力 期望6.0 实际 null；类似业绩/项目负责人 项名未匹配需人核漂移） | |
| 报价勾稽 | **0/3 命中**（合计 1,316,033.66 未出现在结论） | |
| 契约重试均值 | 未观测到重试信号（三跑一次过），记 0 待重核口径 | |

**对照 v2 五节达标线**：召回 29% ↔ 目标 P0≥3/4（D1 已必中 ✅）；客观分 0/3 ↔ 目标 4/4；
manual 6 ↔ 目标 ≤2；墙钟 ✅。差距全部指向 D1（证书扫描页空）与 D2（检索错位过薄）——
与纠偏令 v2 预判一致，Step 4/5 的靶子由此定格。

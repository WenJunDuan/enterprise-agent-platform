...
- **2026-06-21 性能轮（实测前置，用户"先做 p0/p1"）**：治"一家公司十几个文件×十几MB OCR 太慢/超时"。P0 `extract_dir` 线程池并行 + P1 `cache.py` 按文件 sha256 缓存（格式无关+引擎指纹）+ 线程安全 `locks.py`（FITZ_LOCK 覆盖 native直读&engine渲染、PADDLE_LOCK 本地paddle/印章；云OCR不锁仍并行）。**codex review REWORK→全修**（fitz锁不全/缓存key缺指纹/put_cached写失败abort整批/本地paddle并发/workers校验 见 reviews/codex-perf-r1.md）。445 passed。commit `32a2c5a`。**P2 backlog**：全局OCR信号量+云429重试、流式hash、缓存TTL、上传即OCR解耦。
- **2026-06-21 前端补丁**：audit 详情"重新审核"移标题区并排；tender"开始分析"乐观跳第三步 `analyzing` 界面（新screen）+ SSE流式输出预留位。commit `12d13a7`。

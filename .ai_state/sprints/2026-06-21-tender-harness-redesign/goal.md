...
- **2026-06-21 codex 代码 review（REWORK→全修）**：codex 抓 5 P1 + 3 P2——删项目漏 guard `accepted`/active compare（孤儿竞态）、cloud purpose 与 design 不一致、score_mode 缺失无兜底、缺 criteria 完整性校验、hash 未补默认、evaluator 输出形状滞后。**全部修复 + 补 7 测试**（accepted/compare 删除守卫、缺 score_mode/容器告警、默认值 hash 兼容）。见 reviews/codex-impl-r1.md。**440 passed/ruff**。**第 1 轮 ship**。

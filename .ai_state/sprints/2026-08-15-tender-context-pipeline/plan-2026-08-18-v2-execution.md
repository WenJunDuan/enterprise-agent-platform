...
> **用户产品裁决（2026-08-19，效力高于本节原文的"薄实验"定位）**：
> ① 评标必须给结论，禁止裸 manual_review——每项三选一（给分/有据判0/待人工+已找到什么
> +缺什么+查过哪里）；报价项必须提取金额+大小写勾稽+声明"价格分待横比合成"
> （单家评标算不出横比价格分是设计，但报价呈现是义务；基线报价勾稽 0/3 即此缺陷）。
> ② 评标程序：S1 规则先全量罗列成清单，S3 逐条销项，每条"要求→证据→页码→判定"四段。
> ③ grep/FS 工具加入评标会话（锁本案 corpus 目录）——agency 从对照实验升格为产品方向，
> 仍以回归闸数字验收（勾稽 0/3→3/3、manual 6→≤2 为达标线）。
> ④（08-19 追加）结论必须含**基本信息块**（`extracted_data.basic_info`，全部从文件内容抓取
> 并带页码）：招标项目名称/编号/招标人、投标公司名（正文抽取并与申报名比对）、
> **应标一致性校验**（投标声称响应的项目 vs 本次招标项目，不一致=重大发现）、报价、资格逐条。
> 底线不变：无证据不编分，A1-A9 仲裁表继续有效。

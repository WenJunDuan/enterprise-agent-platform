...
### F1+F3 已修并合入（`1ea2954`+`87698f6`，merge `908abac`）
索引 `rag_chunks`/`rag_chunk_scan` 各加 `source_file` 列存**原始 `### 文件:` 头串**（含
`kind=`/`route=`/`[⚠清晰度低]`/`[⚠页号存疑]` 标记，回查闸的降级信号一并保住）；`file` 列
仍为层标识 `__bid__`/`__tender__`，S8 限层过滤与 `following_rows` 同层约束不变。
`build_chunks` 改逐源文件独立建块再按文件序拼接（chunk_id 多源带 `@i` 前缀防撞，单源时
与改前逐字相同），块与源文件一一对应；`EvidenceBlock.render` 输出自洽片段（层标记 +
文件头 + 独占行页锚 + 章节 + 正文）。命令侧补两种底稿形态、片段出处照抄本片段、S0 清单
以在场片段头为准。真红 7 条（`parse_corpus` 只解析出 1 段、商务标块整段被丢弃、招标块挂到
投标文件名下、tier 退化 whole），绿 225 passed（含 corpus/rag/rag_store 三个共享模块的消费面）。

### F9 [P2，新发现，defer polish] 续接可跨源文件滑动
- `server/stores/rag_store.py:171 following_rows` 只按**层** `file`（`__bid__`）过滤，一家投标
  含多份源文件时，中间无命中块的情况下可从甲文件末页滑进乙文件首页。
- **归属不受影响**（F1 修复后每个续接块自带 `source_file`，渲染时各自带文件头），模型也看得见
  文件头变化；问题是相关性上的一次出处跳跃 + 白耗该项额度。
- 修法明确（`following_rows` 加 `source_file` 同源约束），但需自己的 TDD 轮；F1/F3 实施时为避开
  与 F2 并行改同一文件而刻意未碰。**F2 已合入，冲突已消失** → 归 polish 执行。

### F10 [P3，记录不修] 章节名在片段里出现两遍
- `rag._chunk_spans` 既有行为使 `chunk_text` 首行重复 `chapter_path`，叠加新增的 `【章节: …】`
  行后看到两遍。改前的 render 同样重复，非本轮引入。观感问题，不影响解析与回查。

### ⚠ 提示词字节余量告急
`.claude/commands/tender-evaluate.md` 现 **39,192 B / 上界 40,000**（余 808 B）。下一次要往命令里
加规则，**必须先删同等体量的重复表述**，不得抬 `tests/test_prompt_budget.py` 的常量
（棘轮机械化，见该测试头注）。

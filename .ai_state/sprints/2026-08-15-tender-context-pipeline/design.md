...
5. **AC16 追加观测项（pass3 F8/F7）**：用**评分项数 ≥15** 的真实标书测一次——本 sprint 全部
   实测基于 9 项那一份，而 `per_item = evidence_tokens / n_items` 在 15 项时跌破
   `MAX_CHUNK_CHARS`(4,000)，届时任何满尺寸 chunk 都装不下、续接完全不发生（F8）。
   先测形态再定修法（候选：不可分单块借用全局额度 / 超长行按字符兜底切分 / 视为固有约束），
   **不得凭合成语料盲改**——其中"借用全局额度"会破坏闭式账目不变量。

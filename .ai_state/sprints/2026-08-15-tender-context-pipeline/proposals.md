# 白名单外发现与待立项（2026-08-19 批次 + 08-20 增补）

## P1 · 评分项「项内混合」拖累判分（2026-08-20 生产实测，新）

criteria 把「常规参数 4 分 + 性能参数 21 分」抽成**一个 `max=25` 的合并项**。生产结论里
模型明确写出常规部分可判（偏离表空白盖章＝无正偏离），但因性能部分证据不可读，
**整项 25 分被拖成 manual_review**——"有据判 0"义务被项内混合结构绑架，不是模型不肯判。
候选修法（择一，需数据）：①S1 抽取时按评分细则子项拆项；②scoring 支持项内分段
（`sub_items[]`，各自 score/pending）；③维持现状并在度量侧按可判子项折算。
**本批不改**（改的是 criteria 语义与 schema，超白名单）。出处：
`prod-check-2026-08-20-conclusion-duty.md` 三节③。

## P2 · 页号偏移＝第五种缺口形态（2026-08-20 生产实测，四列归因表未覆盖）

同一单实测 `evidence_resolution`：checked 25 / resolved 24 / unresolved **0**，
但 `page_mismatch_detected 7 / page_unverified 7 / page_corrected **0**`——
**引文全在、页号全对不上且一处未纠正**；结论把「开标一览表」引到 P11，v2.2 试跑实测其在 P5。
既非列A（检索脱靶）也非列B（像素不可读），是**页坐标系本身错位**（疑与损坏 PDF 的
qpdf 修复 / 转换稿页锚双坐标系有关）。Step 5 归因表若照四列记账会把它错记进列A。
→ 待 Step 5 定性后决定是否增列E。出处：同上，三节④。
...
## P1 · criteria 抽取无自动重试——坏骰子直接外包给用户手（2026-08-19 实测确诊）

同一份招标 .doc（546,816B）：11:38 抽取产出非法满分（`item_max_invalid`）→ 整单硬失败；
11:41 用户手动重传，同文件抽取通过。本质：**单次采样的概率性组件被当确定性组件用**，
校验器拒收正确（断尺不能量分），缺的是重掷环。修法三件：①结构校验不过→具体错误喂回
模型自动重抽（上限 2 次）；②失败文案分家——结构非法≠扫描件缺文字层，现文案归因错位；
③归一层收严格数字字符串型满分（`"10"`→10），含糊形态（`"10-15分"`）仍走重试不猜。
落点 `doc_pipeline.py` + 测试，半小时刀。用户已知，开工待其点头或下会话顺手做。

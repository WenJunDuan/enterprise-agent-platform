...
发现**原计划的纯比例阈值在本案是 no-op**：ZJ 59 扫描页 / 400 页 = ratio **0.147**，低于原拟
阈值 0.4/0.8 → 整份云 OCR **根本不触发**，与验收（59 页进底稿）自相矛盾。根因：ratio 是
*整份几乎全扫描* 文档的判据；混合 PDF（大量数字页稀释比例）的真信号是**扫描页绝对数量**。

**用户已拍板（2 决策）：**
1. **触发判据 = 计数为主 + 比例兜底**：`blank_count ≥ OCR_BLANK_PAGE_MIN_COUNT(默认10) OR blank_ratio > OCR_BLANK_PAGE_RATIO(默认0.5)`。ZJ 59 页触发；数字 PDF 个别签章页不触发；小份多扫描件靠比例兜底。两阈值 env 可灰度。
2. **Layer 2（逐页并发）本 Sprint 一起做**——但**前置 gate：先实测 aistudio 单页 PNG 接口兼容性**（需 OCR 云凭证+网络），未通过则停在 Layer 1（整份云 OCR 已能让ZJ出真分）。
...
ZJ 3 模型 dogfood 实测：投标 400 页中 **~59 页（资质/业绩/职称/社保/检测报告）是扫描件**。
...
## 验收
- ZJXH重评：59 扫描页内容进底稿，技术参数/企业实力/负责人从 manual_review → **scored 真分**（3 模型，尤其 qwen/deepseek 不再因读不到证书 punt）。
...
- **Phase 0（直接管道）**：ZJXH 45MB/400 页，classify mixed_pdf=True，60 扫描页（ratio 0.150

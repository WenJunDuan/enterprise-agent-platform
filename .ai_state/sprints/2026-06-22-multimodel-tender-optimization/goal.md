...
- R4：🟡 **性能优化（用户2026-06-22扩定：D降重试+A前端上传即OCR+B/C并发）**。
  ...
  - **A 前端上传即OCR ✅**（用户选「每区一次多选→自动传+OCR→锁定」）：addTenderFile 选招标即建项目+上传+OCR；addBidderFile 选某家即传该家(招标未传则拒)；去「上传文件」按钮仅留「开始分析」；投标区在招标传前禁用(强制招标先传)；各区传后锁定(删该家可重选)。移除批量 startUpload 死代码。前端 lint/build/17tests 绿。**⚠️ 视觉需用户 dev 眼验**。

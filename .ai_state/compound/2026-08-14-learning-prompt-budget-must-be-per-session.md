...
已回滚 `tender-evaluate.md` 至重构前单文件形态；**恢复与否待部署机复测**（截至落档时用户尚未跑
回滚后的验证单——"回滚即恢复"只是预期，不是证据）。

**因果归因未闭环**：事故日志显示云 OCR 超时先把请求打到 `inline_ocr` 降级路径（整目录全文注入，
该路径重构前即存在、无长度上限），脚手架 +13.8% 与 OCR 降级灌大 **哪个是主因未做对照实验**
（原定"还原旧命令跑同一案卷"的 A/B 被回滚决策取代）。"改之前是好的"可能只是因为当时云 OCR 没坏。
推论：**即使回滚后，云 OCR 再次超时仍可能复现爆窗**——OCR 降级路径的预算闸与
`Prompt is too long` 不重试这两个修复独立于本次回滚，仍需排期。

...
pointers:
  ...
  latest_architecture_update: "2026-06-21T14:17:06.271Z"

# === PACE 联动字段 (hook 自动维护) ===
next_action: "本会话(2026-06-21):tender评标质量goal,**17 commits未push**(./deploy/deploy.sh dev <新tag>;前端另build)。第1-2轮见git log(criteria提取从2项→20项精准[C根因+cowork G1-G7]/A absence-not-zero/OCR性能轮)。**第3轮G5完整**(3832342):formula公式变量结构化formula_spec(限价类价格分单家可算;tag按variables.source白名单派生[全tender_constant+bid_component才scored,任一cross_bid/external/live/derived→manual];拆子项优先/阶梯走banded/MVP窄固定限价;S2回填本家bid_component;S3以formula_spec为准代入+basis逐步列计算+缺spec不fallback心算);output_contracts三重兜底warning(no_spec/not_closeable/missing_value);criteria横比指纹剥formula_spec.variables[].value/ref保留expression标准侧。codex+CC双critic APPROVE-WITH-CHANGES+impl review REWORK→全修。**第4轮用户3点新需求**:①thinking effort(57e5b6b+06d0197):build_options加effort,codex r4纠全局xhigh拖慢audit→改【tender per-call xhigh(TENDER_REASONING_EFFORT)】audit不受影响+统一校验②logs三项(54ae1d8):清噪音(access filter正则exact path+status只过滤2xx/3xx前缀边界)+落思考日志(tender_progress带request_id)+轮转(复用RotatingFileHandler)③思考流式(轮询伪流式,54ae1d8+06d0197):run_agent_json加on_progress(TextBlock片段,文本模式思考在内)→command_adapter透传→tender_worker flusher每1.5s节流写progress_message(finally cancel+await suppress CancelledError)→前端analyzing progressByRid【按request_id存防多bidder覆盖】替换式+自动滚动。task_store加update_progress(仅running)。codex r4 REWORK→全修3P1+3P2。**471 passed/ruff/前端lint+build**。**思考流式实测验证成功**(progress_message实时捕获agent输出,两次dogfood)。**关键:用户模型=claude-opus-4-8[1M]非deepseek**(之前deepseek-v4-pro[1m]是写错的模型名已改;故effort对Claude原生extended thinking有意义);effort改善评标待端点不429时验证(claude-opus-4-8[1M]网关现Service Unavailable)。dogfood:项目级POST /tender/projects/{id}/evaluate(无绑定/tender/evaluate要__unbound__路径;素材data/submissions/default/tender/tp-f856d66c0e244467/case,绝对路径+mode字段;token=TENANT_KEYS JSON的.default)。**第5轮(用户实测反馈)**:mac mini SSH诊断(直连100.107.151.115 ConnectTimeout30+StrictHostKeyChecking=no,域名.ts.net解析超时改用IP;serve现已停;部署是rsync非git):**评标537s(9min),慢=OCR串行+模型,"审核超时"是体感**(前端被绑mutation await评标+前端轮询DEFAULT_POLL_TIMEOUT_MS超时→onError掉回第二步)。mac mini跑deepseek-v4-pro[1M]/TENDER_TIMEOUT_SEC=1200。**A+B长任务体验解耦**(142ae18,19commits,471passed+前端lint/build,见r5-longtask-ux-ocr-decouple-design.md):前端submitReview只建项目+提交不await评标→独立轮询useQuery(2.5s)拉task喂progressByRid流式→全终态跳分析中心(hasCompare触发triggerTenderCompare);activeEval存localStorage(可离开/回来恢复,screen lazy init readActiveEval);onError只管提交失败不再超时掉回;set-state-in-effect块disable(react-query v5无onSuccess);后端TENDER_TIMEOUT_SEC默认600→3600仅防无限挂兜底。codex review A+B进行中。**待用户拍C范围**:上传即OCR解耦需拆"上传文件"与"提交评标"两步(现evaluateTenderProjectUpload合一→OCR仍评标时跑;拆后选文件即后台OCR预热content-sha256缓存[性能轮已加],点分析命中秒过)。每轮codex配合。**第6轮C(用户选P2+P3一次到位+"自起后端测试监视日志")**:generator subagent worktree实施后端P2(设计r6-three-layer-ocr-decouple-design.md):三层tender_doc_store[tender_project_docs项目级+tender_bid_docs每家]+上传端点[POST projects/{id}/tender-doc与/bids落盘+asyncio后台OCR预热content-sha256缓存,GET docs-status轮询]+评标读层[tender_worker._load_doc_layer_context优先读层,任一未ready/异常→回落ocr_preprocess_block兜底,TENDER_READ_DOC_LAYER开关],43新测试,merge回主514passed+ruff,worktree已清。**端到端实测成功**(自起后端调接口看日志):上传招标79页→15s ready(底稿118K字符落库)/投标158页→22s ready(50K)/提交评标serve.log无回落=读层命中OCR秒过(砍掉537s里OCR串行等待)。22commits。**P2待收尾**:codex review P2(未做)/criteria项目级回填(评标后update_project_doc_criteria后续家复用治每家重复解析)/delete_project_cascade扩展清两新表。**P3未做**:前端拆'上传文件'(选文件即调tender-doc/bids上传OCR)与'开始分析'两步+docs-status轮询+三区布局(基本信息/OCR识别区[招标要点+扣分点/投标要点]/投标文件流式)。**待用户定**:接着前端P3 vs先codex review P2。后端serve在跑(bexb628ov;评标104c834f在跑claude-opus可能429)。backlog:compare首次横比refetchInterval停(codex r5 P1-5未修);effort端点429重测;招标人侧合规MVP;OCR置信度深化;前端deploy未跟踪。"

...
# === PACE 路由状态 ===
path: "Feature" # 2026-07-23 用户插入 X1(环评域,已 done)+X2(tender 名称留存,进行中);D9 System sprint 挂起待 mac mini 回传,状态存 next_action▽存档
stage: "impl" # 2026-07-23 X2: design 三轮 critic 定稿;T1-T3 在分支 worktree-agent-a9ea79174969a5183 未 merge(实为 sonnet 所写,模型违约待用户拍板);T4 未跑;X1 已收口(merge 2d8d822+review PASS,同样 sonnet 违约待拍板)
current_sprint_slug: "2026-07-23-tender-case-header" # X2 tender 案卷头留存(用户插入,与 X1 并行);X1=2026-07-23-eia-domain-page 已 done;D9=2026-07-20-streaming-ocr 挂起待外部
...
# === Pointers (指向最新相关文件) ===
pointers:
  latest_design: "sprints/2026-07-23-tender-case-header/design.md" # X2 三轮 critic 定稿(bidder_info 契约+bid_id 补链+只填空回填);X1 design 见 sprints/2026-07-23-eia-domain-page/
...
last_critic_round: 3 # X2 design: R1 五条(P0 锚点/P0 results 链)→R2 F6 P0(bids 表=tender_bid_docs 非 tender_project_store)+F7 P1(bid_id join key 缺失,手填优先空转)→R3 PASS(2 P2 建议已并入:command_adapter 具名透传+bid_id=None 退化测试)。(X1 两轮/D9 两轮见各 sprint 档案)

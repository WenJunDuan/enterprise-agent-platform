...
def test_running_extraction_is_worth_waiting_for():
    ...
    assert criteria_pending(_doc("running"), STALE_SEC) is True
    assert criteria_pending(_doc("pending"), STALE_SEC) is True


def test_terminal_status_releases_immediately():
    """ready/failed 都是终态——failed 也不该再等（等下去不会变好）。"""
    assert criteria_pending(_doc("ready"), STALE_SEC) is False
    assert criteria_pending(_doc("failed"), STALE_SEC) is False


def test_stale_running_is_not_worth_waiting_for():
    """进程重启遗留的僵尸 running：心跳陈旧 → 按"不会有了"处理，别白等到上限。"""
    assert criteria_pending(_doc("running", age_sec=STALE_SEC * 3), STALE_SEC) is False


def test_missing_or_malformed_rows_never_block():
    """缺行/坏时间戳一律放行——等待是优化，不能变成新的卡死点。"""
    assert criteria_pending(None, STALE_SEC) is False
    assert criteria_pending({"criteria_status": "running"}, STALE_SEC) is False
    assert criteria_pending({"criteria_status": "running", "updated_at": "坏值"}, STALE_SEC) is False

"""CLI 测试：evaluate-bid / evaluate-bid-json 子命令（镜像 audit / audit-json）。
...
def test_evaluate_bid_commands_registered() -> None:
    assert runner.invoke(cli.app, ["evaluate-bid", "--help"]).exit_code == 0
    assert runner.invoke(cli.app, ["evaluate-bid-json", "--help"]).exit_code == 0
...
def test_evaluate_bid_forwards_to_command_adapter(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = runner.invoke(
        cli.app, ["evaluate-bid", "cases/r2024007", "--conversation-id", "conv-eval"]
    )
    ...
    assert command_name == "evaluate-bid"
...
def test_evaluate_bid_json_uses_audit_result_schema(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = runner.invoke(
        cli.app,
        ["evaluate-bid-json", "cases/r2024007", "--conversation-id", "conv-eval-json"],
    )
